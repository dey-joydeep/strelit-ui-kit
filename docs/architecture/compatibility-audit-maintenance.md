...
The audit is a disposition ledger, not a permanent parity requirement. Its role
depends on the active phase in the
[product evolution policy](./product-evolution-policy.md). A baseline item may
be preserved, replaced, migrated, diagnosed, or intentionally removed with an
explicit rationale. During the current Bridge phase, the audit remains part of
normal high-risk verification.
...
An approved phase transition may change where this audit runs or which
dispositions are acceptable. That transition must update the policy,
verification pipeline, support windows, and migration ownership together; an
elapsed review date does not change audit behavior automatically.
