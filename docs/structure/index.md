...
The following diagram shows structure of a Strelit Layout object.

```
+-----------------+      +------------+     +---------------+
| Layout Manager  +------+ GroundItem +-----+ ComponentItem |
| (StrelitLayout   |      | (hidden)   |     | (root item)   |
|  or             |      +------------+     +---------------+
|  VirtualLayout) |                    OR
|                 |                         +---------------+
|                 |                    -----+ Stack         +---+
...
If the root is a `ComponentItem`, then the layout will only have one component/element which the user cannot manipulate with Strelit Layout in any way. To the user, it will appear as if Strelit Layout was not present - just the component/element.
