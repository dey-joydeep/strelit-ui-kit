...
Strelit Layout binds to components and then controls their position, size and visibility (positioning) so that they fit within a layout. There are 4 ways Strelit Layout can bind to a component:

1. **Embedding via Registration** (classic)\
   A component's constructor/factory is registered with Strelit Layout. Strelit Layout will instantiate an instance when required. The constructor/factory will add the component's root HTML element within Strelit Layout's own DOM hierarchy sub-tree. This is the classic Strelit Layout binding method. With this binding method an ancestor of the component's root HTML element could be reparented if the layout changes.
1. **Embedding via Events**\
   Components are obtained on demand by events. An event handler will construct or otherwise fetch the component and return it. The event handler will also add the component's root HTML element within Strelit Layout's own DOM hierarchy sub-tree. This is the event-driven binding method introduced in the TypeScript-based Strelit line.
1. **Virtual via Registration**\
   A component's constructor/factory is registered with Strelit Layout. Strelit Layout will instantiate an instance when required. The component will use the same positioning as virtual components however Strelit Layout will handle all the events internally.
1. **Virtual via Events** (Virtual Components)\
   With virtual components, Strelit Layout never interacts directly with components. The application controls the construction/allocation, destruction/deallocation and positioning of components. Strelit Layout will advise the application when components are needed and no longer needed via events. It will also advise about components' positioning via events. This allows an application to control the placement of components in the DOM hierarchy and components' root HTML element's ancestors are not reparented when the layout is changed.

## Embedding via Registration

Registering a component and specifying static positioning is the classic StrelitLayout approach to binding components. The components are registered with StrelitLayout and specify a constructor or callback function used to create a component whenever a new instance is needed in the layout. When the constructor or callback is invoked, it is passed a container object which includes a HTML element. The constructor or callback will create the object and make its top level HTML element a child of the container's HTML element. The component is then part of the Strelit Layout's DOM hierarchy. Whenever the layout is re-arranged, the StrelitLayout DOM is adjusted to reflect the new layout hierarchy. Effectively this involves the ancestors of components' root HTML elements being reparented when a layout is changed.
...
- Applications using frameworks with their own component hierarchy, such as Angular and Vue, no longer have to break their component hierarchy to insert Strelit Layout. The framework's methodology for handling parent/child relationships can be maintained even with the components which Strelit Layout is positioning. (Teleporting component's HTML elements is no longer necessary)
- Applications typically bind a component's top level HTML element to the Strelit Layout root element. Debugging becomes easier as the DOM hierarchy relevant to your application is a lot shallower.
...
- `VirtualLayout.bindComponentEvent: (container, itemConfig) => ComponentContainer.BindableComponent`\
  Fired whenever a StrelitLayout wants to bind to a new component. The handler is passed the container and the item's resolved config. Typically, the handler would:
    ...
    - make the element a child of Strelit Layout's root HTML element,
...
- `VirtualLayout.unbindComponentEvent: (container) => void`\
  Fired when a component is removed from Strelit Layout. The handler is passed the container. Typically, the handler would:
    - find the component in the map using `container` as the key,
    - remove it as a child from Strelit Layout's root HTML element,
...
- `LayoutManager.beforeVirtualRectingEvent: () => void`\
  This event does not need to be handled. However it can be used to optimise positioning of components. Whenever a layout is changed, it may be that several components need to be repositioned. This event will be fired whenever one or more components need to be positioned as the result of one layout change. Typically it is used to get the position of Strelit Layout's root HTML element, using `getBoundingClientRect()`. This can then be cached for use when each component's position needs to be calculated.

    Example:

    ```typescript
    private handleBeforeVirtualRectingEvent(count: number) {
        this._strelitLayoutBoundingClientRect = this._layoutElement.getBoundingClientRect();
    }
    ```
...
- `ComponentContainer.virtualRectingRequiredEvent: (container, width, height) => void;`\
  Fired when a component's position and/or size need to be changed. The handler is passed the container and the component's required width and height. Typically, the handler would:
    - find the component in the map using `container` as the key,
    - get the Strelit Layout's root HTML element's position using `getBoundingClientRect()`, (Alternatively, it can used the position calculated by the handler for the `virtualRectingRequiredEvent` event.)
    - get the container's position using `getBoundingClientRect()`,
    - calculate the container's position relative to Strelit Layout's root HTML element position.
    ...
    private handleContainerVirtualRectingRequiredEvent(container: ComponentContainer, width: number, height: number) {
        ...
        const left = containerBoundingClientRect.left - this._strelitLayoutBoundingClientRect.left;
        rootElement.style.left = this.numberToPixels(left);
        const top = containerBoundingClientRect.top - this._strelitLayoutBoundingClientRect.top;
...
When using virtual components, think of Strelit Layout as more of an engine calculating position rather than actually positioning components. This binding method requires more work to set up than other binding methods. However it offers more flexibility and opens up more design opportunities. For example with virtual components, any HTML element could be the parent for your components (not just the Strelit Layout container). You can even have different parents for different components. This allows, for example, some of your components have one parent, and the others a different parent. They could then inherit different CSS or handle event propagation differently.

## Virtual via Registration

These events give applications a lot of flexibility with positioning components in Strelit Layout - but at the expense of more effort of integrating into Strelit Layout. It is however, possible to get the same benefits of Virtual Components with just registering a component. In this case, a component will be registered as in classic approach to Strelit Layout binding, however, within Strelit Layout, the component will be handled like a virtual component. Strelit Layout will internally handle the necessary events.
...
With these changes, applications can continue to use Strelit Layout as they are now however Strelit Layout will internally use virtual component binding.

Please note there will be a couple of minor behaviour changes:

- Strelit Layout will ensure a component's root HTML element has position type `absolute`.
...
The inheritance hierarchy for the Strelit Layout class is: `LayoutManager` -> `VirtualLayout` -> `StrelitLayout`.

The `VirtualLayout` class implements all the Strelit Layout functionality except for the register functions. If you only intend to use virtual components using the `bindComponentEvent`, you can create an instance of `VirtualLayout` instead of `StrelitLayout`.
