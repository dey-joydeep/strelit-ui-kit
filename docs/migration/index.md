...
This repository is a Strelit product line, not an upstream "version 2" release.

The current Strelit codebase is a substantial departure from the older Golden Layout 1.5.9 lineage. The major changes can be summarised as:

1. The code has been ported to TypeScript
1. The primary focus of maintenance will be on reliability.

Before migrating from older Golden Layout based applications or earlier internal snapshots, it is important to review the following:

For a practical rename/codemod workflow, see [Golden Layout To Strelit Migration](./golden-layout-to-strelit.md).
For area-by-area parity assessment, see [v1 To v2 Feature Matrix](./v1-to-v2-feature-matrix.md).

## Dropped Features

As part of the port and modernization work, the code base was significantly refactored. A number of older features were dropped because their implementation was not robust enough for the current reliability goals. The dropped features are:

- **React Support** - The [FlexLayout](https://github.com/caplin/FlexLayout) library has been designed for React components. We recommend developers using React to use this library instead of Strelit Layout.
- **Nested Stacks** - While it was possible to create layouts with nested stacks in the older product line, the implementation was incomplete. Due to the amount of work that would have been necessary to fix it, this feature was dropped. Strelit UI Kit does not allow nested stacks.
...
- **No JQuery** - JQuery is no longer used in Strelit Layout (many would consider this as an added feature)

## Migration scope

The current Strelit codebase has been rewritten in TypeScript and then further modernized as a standalone product line.

Also, some changes have been made to the StrelitLayout API. Where possible, backwards compatibility has been retained,however functions and properties kept for backwards compatibility have been marked as deprecated. It is strongly recommend applications be migrated to the new API.

## Config

Configs are now strongly typed. In addition, StrelitLayout now has "Configs" and "Resolved Configs"
...
1. Configs\
   ...
   Config parameters in StrelitLayout API methods will be of type "Config". The one exception is `LayoutConfig.saveLayout()` which returns a "Resolved Config".
1. Resolved Configs\
   Golden-Layout internally uses "Resolved Config"s. Whenever an API function is passed a "Config", StrelitLayout will resolve it to its corresponding "Resolved Config". This resolving process will set default values where an optional value has not been specified. It will also handle backwards compatibility. This allows the StrelitLayout library to always work with fully configured Configs.
...
The `ComponentItemConfig.componentName` property has now been replaced by `ComponentItemConfig.componentType`. `componentType` is of type `ComponentType`. While a component type can now be any serializable value, it must be a `string` if it is registered with one of the following functions:

1. `StrelitLayout.registerComponent()` (deprecated)
1. `StrelitLayout.registerComponentConstructor()`
1. `StrelitLayout.registerComponentFactoryFunction()`
...
## StrelitLayout class and VirtualLayout class

`StrelitLayout` is now a distinct class which is a descendant of the `VirtualLayout` class, which in turn is a descendant of the `LayoutManager` class. Your application should always create an instance of either `StrelitLayout` or `VirtualLayout`.

The `StrelitLayout` and `VirtualLayout` constructors takes 3 optionals parameters:

1. The HTML element which contains the StrelitLayout instance. If this is not specified, StrelitLayout will be placed under `body`.
...
The StrelitLayout class now handles component registration. LayoutManager no longer includes any component registration functions. The following changes to registration functions have been made:
...
1. Do not construct an instance of LayoutManager. Construct an instance of StrelitLayout (see above).
1. Do not call `init()`. Call `LayoutManager.loadLayout()` instead.
1. `loadLayout()` (new function)\
   Will load the new layout specified in its `LayoutConfig` parameter. This can also be subsequently called whenever the StrelitLayout layout is to be replaced.
...
1. `setSize()` (new function)\
   Sets the size of the StrelitLayout instance in pixels. Replaces the existing `updateSize()` function.
...
1. `config` property has been removed. Use the toConfig() method instead (as recommended in the original StrelitLayout documentation).
...
1. `Component` has been renamed to `ComponentItem`. "Component" now refers to the external component hosted inside StrelitLayout
...
1. `stateRequestEvent` (new event)\
   If set, `stateRequestEvent` is fired whenever StrelitLayout wants the latest state for a component. Calling `LayoutManager.saveLayout()` will cause this event to be fired (if it is defined). If it is not defined, then the initial state in the ItemConfig or the latest state set in `setState()` will be saved.
...
1. `strelit-ui-kit-untrimmed.d.ts` which contains all (public and internal) API elements. Use this declaration file if you wish to access any API element in the library however please take the above warning into account.
