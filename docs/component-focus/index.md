...
A focused component's tab and header HTML elements will contain the class `strelit_focused`. This can be used to highlight the focused tab and or header. The `strelit-dark-theme.less` theme that ships with Strelit Layout (and is used by `apitest`) will set the background color of a focused tab to a different color from other tabs. If you do NOT want focused tabs to be highlighted, ensure that the `strelit_focused` selector is removed from the relevant css/less/scss used by your application.
