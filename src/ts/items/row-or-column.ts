...
/** @internal */
export interface RowOrColumnAbsoluteSizes {
    itemSizes: number[];
...
/** @internal */
export function getRowOrColumnElementDimensionSize(
    element: HTMLElement,
...
/** @internal */
export function setRowOrColumnElementDimensionSize(
    element: HTMLElement,
    dimension: WidthOrHeightPropertyName,
    value: number,
): void {
...
/** @internal */
export function createRowOrColumnElement(
    document: Document,
    isColumn: boolean,
): HTMLDivElement {
...
export class RowOrColumn extends ContentItem {
    ...
    constructor(
        ...
        super(
            ...
            config,
            _rowOrColumnParent,
            createRowOrColumnElement(document, isColumn),
        );
    ...
    override addChild(
        ...
        for (let i = 0; i < this.contentItems.length; i++) {
            ...
            if (indexedContentItem === contentItem) {
                ...
                const itemSize = (indexedContentItem.size *=
                    (100 - newItemSize) / 100);
    ...
    toConfig(): ResolvedRowOrColumnItemConfig {
        const result: ResolvedRowOrColumnItemConfig = {
            ...
            content: this.calculateConfigContent() as (
     ...
     * @internal
     */
    private calculateAbsoluteSizes(): RowOrColumnAbsoluteSizes {
        ...
        if (Math.round(total) === 100) {
            ...
            if (Math.round(total) < 100 && itemsWithFractionalSize.length > 0) {
                ...
                if (
                    ...
                    itemsWithFractionalSize.length > 0
                ) {
                    for (let i = 0; i < itemsWithFractionalSize.length; i++) {
                        const contentItem = itemsWithFractionalSize[i];
                        contentItem.size =
                            50 * (contentItem.size / totalFractionalSize);
                        contentItem.sizeUnit = SizeUnitEnum.Percent;
                    }
                    total += 50;
                }
        ...
        if (minItemSize <= 0 || this.contentItems.length <= 1) {
            ...
            if (totalUnderMin === 0 || totalUnderMin > totalOverMin) {
                ...
                for (let i = 0; i < this.contentItems.length; i++) {
                    const contentItem = this.contentItems[i];
                    contentItem.size =
                        (allEntries[i].size / absoluteSizes.totalSize) * 100;
                }
            }
        }
    }
    ...
    private onSplitterDragStart(splitter: Splitter) {
        ...
        const beforeWidth = pixelsToNumber(
