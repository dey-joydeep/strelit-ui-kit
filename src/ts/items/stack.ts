...
import {
    ...
    eventEmitterHeaderClickEventName,
...
export class Stack extends ComponentParentableItem {
    ...
    constructor(
        ...
        super(layoutManager, config, parent, createStackElement(document));
        ...
        let componentHeaderConfig: ResolvedHeaderedItemConfigHeader | undefined;
        ...
        this._header = new Header(
            ...
            (x, y, dragListener, item) =>
                this.handleHeaderComponentStartDragEvent(
                    x,
                    y,
                    dragListener,
                    item,
    ...
    override init(): void {
        ...
        for (let i = 0; i < this.contentItems.length; i++) {
            this._childElementContainer.appendChild(
    ...
    toConfig(): ResolvedStackItemConfig {
        ...
        if (this._activeComponentItem) {
            activeItemIndex = this.contentItems.indexOf(
                this._activeComponentItem,
            );
        ...
        if (this.contentItems.length > 0 && activeItemIndex === undefined) {
            ...
            const result: ResolvedStackItemConfig = {
                type: 'stack',
                content:
                    this.calculateConfigContent() as ResolvedComponentItemConfig[],
                size: this.size,
    ...
    /**
     * Ok, this one is going to be the tricky one: The user has dropped a `ContentItem` onto this stack.
     *
    ...
    override onDrop(contentItem: ContentItem, area: ContentItemArea): void {
        /*
         * The item was dropped on the header area. Just add it as a child of this stack and
         * get the hell out of this logic
         */
        if (this._dropSegment === StackSegment.Header) {
         ...
         */
        const isVertical =
            this._dropSegment === StackSegment.Top ||
            this._dropSegment === StackSegment.Bottom;
        const isHorizontal =
            this._dropSegment === StackSegment.Left ||
            this._dropSegment === StackSegment.Right;
        const insertBefore =
            this._dropSegment === StackSegment.Top ||
            this._dropSegment === StackSegment.Left;
        ...
        if (hasCorrectParent) {
            ...
            const itemConfig = createResolvedItemConfigDefault(
                type,
    ...
    override highlightDropZone(x: number, y: number): void {
        for (const key in this._contentAreaDimensions) {
            const segment = key as StackSegment;
            const area = this._contentAreaDimensions[segment].hoverArea;

            if (area.x1 < x && area.x2 > x && area.y1 < y && area.y2 > y) {
                if (segment === StackSegment.Header) {
                    this._dropSegment = StackSegment.Header;
                    this.highlightHeaderDropZone(
                        this._header.leftRightSided ? y : x,
                    );
                } else {
                    this.resetHeaderDropZone();
    ...
    private updateNodeSize(): void {
        if (this.element.style.display !== 'none') {
            const content: WidthAndHeight = getElementWidthAndHeight(
                this.element,
            );
            ...
            this._childElementContainer.style.width = numberToPixels(
                content.width,
            );
            this._childElementContainer.style.height = numberToPixels(
                content.height,
            );
            for (let i = 0; i < this.contentItems.length; i++) {
    ...
    /** @internal */
    private highlightBodyDropZone(segment: StackSegment): void {
    ...
    private handleHeaderClickEvent(ev: MouseEvent) {
        const eventName = eventEmitterHeaderClickEventName;
        const bubblingEvent = new ClickBubblingEvent(eventName, this, ev);
        this.emit(eventName, bubblingEvent);
    }

    /** @internal */
    private handleHeaderTouchStartEvent(ev: TouchEvent) {
        const eventName = eventEmitterHeaderTouchStartEventName;
        const bubblingEvent = new TouchStartBubblingEvent(eventName, this, ev);
        this.emit(eventName, bubblingEvent);
    }
    ...
    private createHeaderConfig() {
        if (!this._headerSideChanged) {
            return createResolvedHeaderedItemConfigHeaderCopy(
                this._headerConfig,
            );
        } else {
            const show = this._header.show ? this._header.side : false;

            let result = createResolvedHeaderedItemConfigHeaderCopy(
                this._headerConfig,
                show,
            );
            if (result === undefined) {
