...
export const StackItemConfig = {
    ...
    fromResolved(resolvedConfig: ResolvedStackItemConfig): StackItemConfig {
        const result: StackItemConfig = {
            ...
            header: createResolvedHeaderedItemConfigHeaderCopy(
...
export const RowOrColumnItemConfig = {
    ...
    resolveContent(
        ...
        if (content === undefined) {
            ...
            const count = content.length;
            const childItemConfigs =
                new Array<RowOrColumnItemConfigChildItemConfig>(count);
            ...
            const result =
                new Array<ResolvedRowOrColumnItemConfigChildItemConfig>(count);
            for (let i = 0; i < count; i++) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            switch (type) {
                ...
                case ItemType.column:
                    ...
                    break;
                case ItemType.stack:
                    contentConfig = StackItemConfig.fromResolved(
                        resolvedContentConfig,
                    );
                    break;
                case ItemType.component:
    ...
    fromResolvedOrUndefined(
        ...
        if (resolvedItemConfig === undefined) {
            ...
            switch (type) {
                case ItemType.row:
                case ItemType.column:
                    return RowOrColumnItemConfig.fromResolved(
                        resolvedItemConfig,
                    );
                case ItemType.stack:
...
export const LayoutConfig = {
    ...
    Dimensions: {
        resolve(
            ...
            const {
                size: defaultMinItemHeight,
                sizeUnit: defaultMinItemHeightUnit,
            } = this.resolveDefaultMinItemHeight(dimensions);
            const {
                size: defaultMinItemWidth,
                sizeUnit: defaultMinItemWidthUnit,
    ...
    fromResolved(config: ResolvedLayoutConfig): LayoutConfig {
        const result: LayoutConfig = {
            ...
            openPopouts: PopoutLayoutConfig.fromResolvedArray(
                config.openPopouts,
            ),
...
export const PopoutLayoutConfig = {
    Window: {
        ...
        fromResolved(
            ...
            const result: PopoutLayoutConfigWindow = {
                width:
                    resolvedWindow.width === null
                        ? undefined
                        : resolvedWindow.width,
                height:
                    resolvedWindow.height === null
                        ? undefined
                        : resolvedWindow.height,
                left:
                    resolvedWindow.left === null
                        ? undefined
                        : resolvedWindow.left,
                top:
                    resolvedWindow.top === null
                        ? undefined
                        : resolvedWindow.top,
            };
    ...
    resolve(popoutConfig: PopoutLayoutConfig): ResolvedPopoutLayoutConfig {
        ...
        const config: ResolvedPopoutLayoutConfig = {
            root: RootItemConfig.resolve(root),
            openPopouts: LayoutConfig.resolveOpenPopouts(
                popoutConfig.openPopouts,
            ),
            dimensions: LayoutConfig.Dimensions.resolve(
    ...
    fromResolved(
        ...
        const result: PopoutLayoutConfig = {
            ...
            window: PopoutLayoutConfig.Window.fromResolved(
                resolvedConfig.window,
            ),
        };
