...
/** @internal */
export type TabsContainerComponentItemRemoveEvent = (
    this: void,
    componentItem: ComponentItem,
) => void;
/** @internal */
export type TabsContainerComponentItemFocusEvent = (
    this: void,
    componentItem: ComponentItem,
) => void;
/** @internal */
export type TabsContainerComponentItemDragStartEvent = (
    this: void,
    x: number,
    y: number,
...
/** @internal */
export type TabsContainerDropdownActiveChangedEvent = (this: void) => void;
...
export class TabsContainer {
    ...
    constructor(
        ...
        private _componentRemoveEvent: TabsContainerComponentItemRemoveEvent,
        private _componentFocusEvent: TabsContainerComponentItemFocusEvent,
        private _componentDragStartEvent: TabsContainerComponentItemDragStartEvent,
        private _dropdownActiveChangedEvent: TabsContainerDropdownActiveChangedEvent,
    ) {
        ...
        this._dropdownElement.classList.add(
            DomConstants.ClassName.TabDropdownList,
        );
    ...
    createTab(componentItem: ComponentItem, index: number): void {
        ...
        if (index < this._element.childNodes.length) {
            this._element.insertBefore(
                tab.element,
                this._element.childNodes[index],
            );
        } else {
    ...
    processActiveComponentChanged(newActiveComponentItem: ComponentItem): void {
        ...
        for (let i = 0; i < this._tabs.length; i++) {
            const isActive =
                this._tabs[i].componentItem === newActiveComponentItem;
        ...
        if (activeIndex < 0) {
            ...
            if (
                this._layoutManager.layoutConfig.settings.reorderOnTabMenuClick
            ) {
                /**
    ...
    updateTabSizes(
        ...
        activeComponentItem: ComponentItem | undefined,
    ): void {
    ...
    tryUpdateTabSizes(
        ...
        if (this._tabs.length > 0) {
            if (activeComponentItem === undefined) {
                throw new Error(
                    'non-empty tabs must have active component item',
                );
            }
