...
export class App {
    ...
    private readonly _registerComponentTypesAsVirtualButtonClickListener = () =>
        this.registerComponentTypesAsVirtual();
    ...
    private readonly _registeredComponentTypesForAddSelectChangeListener = () =>
        this.handleRegisteredComponentTypesForAddSelectChange();
    private readonly _eventBindingVirtualRadio;
    private readonly _eventBindingVirtualRadioClickListener = () =>
        this.handleEventBindingVirtualRadioClick();
    private readonly _eventBindingEmbeddedRadio;
    private readonly _eventBindingEmbeddedRadioClickListener = () =>
        this.handleEventBindingEmbeddedRadioClick();
    private readonly _clearButton: HTMLButtonElement;
    private readonly _clearButtonClickListener = () =>
        this.handleClearButtonClick();
    private readonly _addComponentButton: HTMLButtonElement;
    private readonly _addComponentButtonClickListener = () =>
        this.handleAddComponentButtonClick();
    private readonly _layoutSelect: HTMLSelectElement;
    private readonly _layoutSelectChangeListener = () =>
        this.handleLayoutSelectChange();
    private readonly _loadLayoutButton: HTMLButtonElement;
    private readonly _loadLayoutButtonClickListener = () =>
        this.handleLoadLayoutButtonClick();
    private readonly _loadComponentAsRootButton: HTMLButtonElement;
    private readonly _loadComponentAsRootClickListener = () =>
        this.handleLoadComponentAsRootButtonClick();
    private readonly _registeredComponentTypesForReplaceSelect: HTMLSelectElement;
    private readonly _registeredComponentTypesForReplaceSelectChangeListener =
        () => this.handleRegisteredComponentTypesForReplaceSelectChange();
    private readonly _replaceComponentButton: HTMLButtonElement;
    private readonly _replaceComponentButtonClickListener = () =>
        this.handleReplaceComponentButtonClick();
    private readonly _saveLayoutButton: HTMLButtonElement;
    private readonly _saveLayoutButtonClickListener = () =>
        this.handleSaveLayoutButtonClick();
    private readonly _reloadSavedLayoutButton: HTMLButtonElement;
    private readonly _reloadSavedLayoutButtonClickListener = () =>
        this.handleReloadSavedLayoutButtonClick();
    ...
    private _strelitLayoutBoundingClientRect: DOMRect = new DOMRect();
    ...
    constructor() {
        ...
        this._strelitLayout = new StrelitLayout(
            this._layoutElement,
            this._bindComponentEventListener,
            this._unbindComponentEventListener,
        );

        this._strelitLayout.resizeWithContainerAutomatically = true;
        this._strelitLayout.beforeVirtualRectingEvent = (count) =>
            this.handleBeforeVirtualRectingEvent(count);
        this._strelitLayout.addEventListener('stackHeaderClick', (event) =>
            this.handleStackHeaderClick(event),
        );

        const registerNotVirtualComponentTypesButton = document.querySelector(
            '#registerNotVirtualButton',
        ...
        this._registerComponentTypesButton =
            registerNotVirtualComponentTypesButton;
        this._registerComponentTypesButton.addEventListener(
            'click',
            this._registerComponentTypesButtonClickListener,
            { passive: true },
        );

        const registerNotVirtualAllRadio = document.querySelector(
            '#registerNotVirtualAllRadio',
        ) as HTMLInputElement;
        ...
        const registerNotVirtualColorEventRadio = document.querySelector(
            '#registerNotVirtualColorEventRadio',
        ) as HTMLInputElement;
        ...
        const registerVirtualComponentTypesButton = document.querySelector(
            '#registerVirtualButton',
        ) as HTMLButtonElement;
        if (registerVirtualComponentTypesButton === null) {
            throw new Error('Could not find RegisterVirtualButton');
        }
        this._registerComponentTypesAsVirtualButton =
            registerVirtualComponentTypesButton;
        this._registerComponentTypesAsVirtualButton.addEventListener(
            'click',
            this._registerComponentTypesAsVirtualButtonClickListener,
            { passive: true },
        );

        const registerVirtualAllRadio = document.querySelector(
            '#registerVirtualAllRadio',
        ) as HTMLInputElement;
        ...
        const registerVirtualTextBooleanRadio = document.querySelector(
            '#registerVirtualTextBooleanRadio',
        ) as HTMLInputElement;
        ...
        const eventBindingVirtualRadio = document.querySelector(
            '#eventBindingVirtualRadio',
        ) as HTMLInputElement;
        ...
        this._eventBindingVirtualRadio.addEventListener(
            'click',
            this._eventBindingVirtualRadioClickListener,
            { passive: true },
        );

        const eventBindingEmbeddedRadio = document.querySelector(
            '#eventBindingEmbeddedRadio',
        ) as HTMLInputElement;
        ...
        this._eventBindingEmbeddedRadio.addEventListener(
            'click',
            this._eventBindingEmbeddedRadioClickListener,
            { passive: true },
        );
        ...
        this._clearButton.addEventListener(
            'click',
            this._clearButtonClickListener,
            { passive: true },
        );

        const registeredComponentTypesForAddSelect = document.querySelector(
            '#registeredComponentTypesForAddSelect',
        ) as HTMLSelectElement;
        if (registeredComponentTypesForAddSelect === null) {
            throw new Error();
        }
        this._registeredComponentTypesForAddSelect =
            registeredComponentTypesForAddSelect;
        this._registeredComponentTypesForAddSelect.addEventListener(
            'change',
            this._registeredComponentTypesForAddSelectChangeListener,
            { passive: true },
        );
        ...
        this._addComponentButton.addEventListener(
            'click',
            this._addComponentButtonClickListener,
            { passive: true },
        );

        const addComponentByDragButton = document.querySelector(
            '#addComponentByDragButton',
        ) as HTMLButtonElement;
        if (addComponentByDragButton === null) {
            throw new Error('Could not find addComponentByDragButton');
        }
        const addComponentDragSource = this._strelitLayout.newDragSource(
            addComponentByDragButton,
            () => this.getDragComponentTypeAndState(),
        );
        ...
        this._layoutSelect.addEventListener(
            'change',
            this._layoutSelectChangeListener,
            { passive: true },
        );

        const loadLayoutButton = document.querySelector(
            '#loadLayoutButton',
        ) as HTMLButtonElement;
        ...
        this._loadLayoutButton.addEventListener(
            'click',
            this._loadLayoutButtonClickListener,
            { passive: true },
        );

        const loadComponentAsRootButton = document.querySelector(
        ...
        this._loadComponentAsRootButton.addEventListener(
            'click',
            this._loadComponentAsRootClickListener,
            { passive: true },
        );

        const registeredComponentTypesForReplaceSelect = document.querySelector(
            '#registeredComponentTypesForReplaceSelect',
        ) as HTMLSelectElement;
        if (registeredComponentTypesForReplaceSelect === null) {
            throw new Error();
        }
        this._registeredComponentTypesForReplaceSelect =
            registeredComponentTypesForReplaceSelect;
        this._registeredComponentTypesForReplaceSelect.addEventListener(
            'change',
            this._registeredComponentTypesForReplaceSelectChangeListener,
            { passive: true },
        );

        const replaceComponentButton = document.querySelector(
        ...
        this._replaceComponentButton.addEventListener(
            'click',
            this._replaceComponentButtonClickListener,
            { passive: true },
        );

        const saveLayoutButton = document.querySelector(
            '#saveLayoutButton',
        ) as HTMLButtonElement;
        ...
        this._saveLayoutButton.addEventListener(
            'click',
            this._saveLayoutButtonClickListener,
            { passive: true },
        );

        const reloadSavedLayoutButton = document.querySelector(
        ...
        this._reloadSavedLayoutButton.addEventListener(
            'click',
            this._reloadSavedLayoutButtonClickListener,
            { passive: true },
        );

        const lastVirtualRectingCountSpan = document.querySelector(
            '#lastVirtualRectingCountSpan',
        ) as HTMLSpanElement;
        ...
        this._lastVirtualRectingCountSpan.innerText =
            this._lastVirtualRectingCount.toString();

        const bubbleClickCountSpan = document.querySelector(
            '#bubbleClickCountSpan',
        ) as HTMLSpanElement;
        ...
        this._bubbleClickCountSpan.innerText =
            this._bubbleClickCount.toString();
        const captureClickCountSpan = document.querySelector(
            '#captureClickCountSpan',
        ) as HTMLSpanElement;
        ...
        this._captureClickCountSpan.innerText =
            this._captureClickCount.toString();

        const stackHeaderClickedDiv = document.querySelector(
            '#stackHeaderClickedDiv',
        ) as HTMLDivElement;
        ...
        const stackHeaderClickedItemCountSpan = document.querySelector(
            '#stackHeaderClickedItemCountSpan',
        ) as HTMLSpanElement;
    ...
    private createComponent(
        ...
        switch (componentTypeName) {
            ...
            default:
                throw new Error(
                    'createComponent: Unexpected componentTypeName: ' +
                        componentTypeName,
                );
        }
    }
    ...
    private handleBindComponentEvent(
        ...
        if (componentTypeName === undefined) {
            throw new Error(
                'handleBindComponentEvent: Undefined componentTypeName',
            );
        }
    ...
    private handleBeforeVirtualRectingEvent(count: number) {
        this._strelitLayoutBoundingClientRect =
            this._layoutElement.getBoundingClientRect();
    ...
    private handleContainerVirtualRectingRequiredEvent(
        ...
        if (rootElement === undefined) {
            throw new Error(
                'handleContainerVirtualRectingRequiredEvent: Component does not have a root HTML element',
            );
        }
    ...
    private handleContainerVirtualVisibilityChangeRequiredEvent(
        ...
        if (component === undefined) {
            throw new Error(
                'handleContainerVisibilityChangeRequiredEvent: Component not found',
            );
        }
        ...
        if (componentRootElement === undefined) {
            throw new Error(
                'handleContainerVisibilityChangeRequiredEvent: Component does not have a root HTML element',
            );
        }
    ...
    private handleContainerVirtualZIndexChangeRequiredEvent(
        ...
        if (componentRootElement === undefined) {
            throw new Error(
                'handleContainerVirtualZIndexChangeRequiredEvent: Component does not have a root HTML element',
            );
        }
    ...
    private registerComponentTypes() {
        ...
        this._strelitLayout.registerComponentConstructor(
    ...
    private handleAddComponentButtonClick() {
        ...
        this._strelitLayout.addComponent(componentType);
    }
    ...
    private handleLoadLayoutButtonClick() {
        ...
        if (selectedLayout === undefined) {
            ...
            this._strelitLayout.loadLayout(selectedLayout.config);
        }
    }
    ...
    private handleLoadComponentAsRootButtonClick() {
        ...
        this._strelitLayout.loadComponentAsRoot(itemConfig);
    }
    ...
    private handleReplaceComponentButtonClick() {
        const componentType =
            this._registeredComponentTypesForReplaceSelect.value;
        const itemConfig: ComponentItemConfig = {
            componentType,
            type: 'component',
        };
        const rootItem = this._strelitLayout.rootItem;
    ...
    private handleSaveLayoutButtonClick() {
        this._savedLayout = this._strelitLayout.saveLayout();
    ...
    private replaceComponentRecursively(
        content: ContentItem[],
